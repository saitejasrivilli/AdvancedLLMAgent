...
def extract_goal_statement(lean_code: str) -> str:
    """
    Extracts the goal statement from the Lean code template.
    Tries to find the most relevant theorem or example block to serve as the proof goal.
    """
    match = re.search(r"(theorem|example)\s+([a-zA-Z0-9_]+).*?:.*?\n.*?\{\{proof\}\}", lean_code, re.DOTALL)
    if match:
        return f"Prove the Lean {match.group(1)} named '{match.group(2)}' as defined in the code template."

    spec_match = re.search(r"def\s+([a-zA-Z0-9_]+_spec.*?)\s*:.*?Prop\s*:=+", lean_code)
    if spec_match:
        return f"Prove that the specification '{spec_match.group(1)}' is satisfied by the implementation."

    func_match = re.search(r"def\s+([a-zA-Z0-9_]+)\s*\(.*?\)\s*:\s*[a-zA-Z0-9_]+\s*:=+", lean_code)
    if func_match:
        return f"Prove the correctness of the function '{func_match.group(1)}' using its specification."

    return "Prove the theorem specified in the Lean code template using Mathlib and Aesop."
